...
async function fetchMovies() {
    ...
    const options = {
        method: 'GET',
        headers: {
            'x-rapidapi-key': '406d173e94msh15b3af1b6f3e811p12992djsn3a842899b0dc',
            'x-rapidapi-host': 'imdb-top-100-movies.p.rapidapi.com'
        }
    };
...
nextBtn.addEventListener('click', () => {
    ...
    if (currentQuestionIndex < questions.length) {
        ...
        if (pickedGenre) {
            ...
            console.log(`Selected Genre: ${genreLabel}`); 

            const filterMovies = movies.filter(movie => {
                console.log(`Checking movie: ${movie.title} for genre: ${genreLabel}`); 
                return movie.genre.includes(genreLabel);
            });

            console.log(`Filtered Movies: ${filterMovies.length}`); 
            if (filterMovies.length > 0) {
                ...
                resultText.textContent = 'The Result:';
